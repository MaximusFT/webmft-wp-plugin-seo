...
class WebMFT_Postviews {
	const OPT_NAME = 'webmft_postviews';
	...
	static function init(){
		if (is_null(self::$inst)) self::$inst = is_admin() ? new WebMFT_Postviews_Admin() : new self;
		return self::$inst;
	}
	...
	function show_js(){
		...
		$force_show = apply_filters('webmft_postviews_force_show_js', false);
		...
		<?php
		$script = apply_filters('webmft_postviews_script', ob_get_clean());

		echo preg_replace('~[\r\n\t]~', '', $script)."\n";

		do_action('after_webmft_postviews_show_js');
	}
}

class WebMFT_Postviews_Admin extends WebMFT_Postviews {
	...
	function edit_tax_fields( $term ) {
		?>
		<tr class="form-field">
			<th scope="row" valign="top"><label for="extra1">Views</label></th>
			<td>
	...
	function add_tax_columns( $columns){
		...
		foreach($columns as $k => $col){
			$_columns[ $k ] = $col;
			if ($i++ == 4 ) $_columns['views'] = '<span title="'. __('Views','mft') .'" class="dashicons dashicons-visibility"></span>';
		}
		return $_columns;
	}

	function fill_tax_columns( $out, $colname, $id){
		if ($colname == 'views') $out = ($v=get_term_meta($id, 'views', 1)) ? $v : '0';
		return $out;
	}
	...
	function settings_link($links){
		array_unshift( $links, '<a href="'.admin_url('options-general.php?page=webmft_postviews').'">'.__('Settings', 'mft') .'</a>' );
		return $links;
	}

	function add_options_page(){
		add_submenu_page( 'options-general.php', 'WebMFT: Plugin SEO useful', 'WebMFT: SEO', 'manage_options', 'webmft_postviews', array(&$this, 'options_page_output'));
	}

	function options_page_output(){
		?>
		<div class="wrap">
			<h2>WebMFT: Plugin SEO useful</h2>

			<form action="options.php" method="post">
				<?php
				settings_fields('option_group');  // скрытые защитные поля
				do_settings_sections('mft_page'); // секции с настройками (опциями). У нас она всего одна 'section_id'
				submit_button();
				?>
			</form>
		</div>
	...
	function register_setting(){
		...
		$section = 'mft_section_1';
		add_settings_section( $section, '', '', 'mft_page' );

		// параметры: $id, $title, $callback, $page, $section, $args
		$id = 'who_count';
		add_settings_field( $id, __('Whose visit count?','mft'), array( &$this, 'fill_field'), 'mft_page', $section, array('id'=>$id, 'desc'=>'' ));

		$id = 'hold_sec';
		add_settings_field( $id, __('Delay in seconds','mft'), array( &$this, 'fill_field'), 'mft_page', $section, array('id'=>$id, 'desc'=>__('How many seconds to delay and then count visit?','mft')));

		//$id = 'wp_config_relpath';
		//add_settings_field( $id, __('Relative path to detect wp-config.php','mft'), array( &$this, 'fill_field'), 'mft_page', $section, array('id'=>$id, 'desc'=>__('Relative path between DOCUMENT_ROOT and wp-config.php <code>DOCUMENT_ROOT{relpath}wp-config.php</code>','mft')));
	}
	...
	function fill_field( $args){
		...
		if ($id == 'who_count'){
			$options = array();
			foreach(
				array('all'=>__('All','mft'), 'not_logged_users'=>__('Only not logged users','mft'), 'logged_users'=>__('Only logged users','mft'), 'not_administrators'=>__('All, except administrators','mft'))
				as $kk=>$vv){
